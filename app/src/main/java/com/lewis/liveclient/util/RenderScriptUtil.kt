...
import com.lewis.liveclient.ScriptC_transform2NV21
...
private var _script: ScriptC? = null
val script: ScriptC get() = _script ?: throw NullPointerException("the script is null In RenderScriptUtil.kt")

fun initRenderScript(isNV21: Boolean) {
  _rs = RenderScript.create(androidApp.baseContext)
  _script = if (isNV21) {
    ScriptC_transform2NV21(rs)
  } else {
    ScriptC_transform2YUV420p(rs)
  }
}

fun getYUV420FrameBufferByRenderScript(script: ScriptC, array: ByteArray, width: Int, height: Int)
    : ByteArray {
  when (script) {
    is ScriptC_transform2YUV420p -> {
    ...
    else -> throw UnsupportedOperationException("the script type is unsupported!")
  }
}
